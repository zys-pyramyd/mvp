...
if not JWT_SECRET:
    import logging
    logging.getLogger(__name__).warning("JWT_SECRET not set. Using insecure default for dev only.")
...
async def forgot_password(request: ForgotPasswordRequest):
    ...
    try:
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
            ...
            <p style="font-size: 16px;">Here is your Pyramyd verification code:</p>
            <div style="text-align: center; margin: 30px 0;">
                <span style="display: inline-block; padding: 12px 24px; background-color: #f3f4f6; color: #000; border: 1px dashed #059669; font-size: 28px; letter-spacing: 5px; font-weight: bold;">{otp}</span>
            </div>
            <p style="font-size: 14px; color: #666;">This code expires securely in 15 minutes.</p>
        </div>
        """
        send_zeptomail(
            to_email=request.email,
            subject="Pyramyd Verification Code",
            html_content=reset_html
        )
        import logging
        logging.getLogger(__name__).info(f"Password reset OTP generated for {request.email}")
    except Exception as e:
        import logging
...
async def startup_event():
    ...
    logging.getLogger(__name__).info(f"Application starting up on port {os.environ.get('PORT', '8001')}...")
    init_db_indexes()
