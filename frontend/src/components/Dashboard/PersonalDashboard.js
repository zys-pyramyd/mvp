...
import { X, ShoppingBag, CreditCard, TrendingUp, Info, ListOrdered } from 'lucide-react';
import MyRequests from '../rfq/MyRequests';
...
const PersonalDashboard = ({ onClose, user, API_BASE_URL }) => {
    ...
    // Fetch Requests when the tab is switched
    useEffect(() => {
    ...
    const fetchMyRequests = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Ensure array, maybe backend returns { requests: [] } or just []
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                {/* Header */}
                <div className="flex flex-col border-b sticky top-0 bg-white z-10 px-6 pt-6 pb-0">
                    ...
                    <div className="flex space-x-6 text-sm">
                        <button
                            className={`pb-3 font-medium border-b-2 transition-colors ${activeTab === 'overview' ? 'text-emerald-600 border-emerald-600' : 'text-gray-500 border-transparent hover:text-gray-800'}`}
                            onClick={() => setActiveTab('overview')}
                        >
                            Overview
                        </button>
                        <button
                            className={`pb-3 font-medium border-b-2 transition-colors ${activeTab === 'requests' ? 'text-emerald-600 border-emerald-600' : 'text-gray-500 border-transparent hover:text-gray-800'}`}
                    ...
                    {activeTab === 'requests' && (
                        <div>
                            {loadingRequests ? (
                                ...
                                <MyRequests requests={requests} onRefresh={fetchMyRequests} />
                            )}
                        </div>
                    )}
                </div>
